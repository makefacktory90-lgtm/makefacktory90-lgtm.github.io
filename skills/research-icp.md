...
**Бюджетный стек (рекомендую):** через OpenRouter, одно исследование ЦА обходится в ~$0.05-0.08.

| Роль | Модель | Цена ($/1M in-out) | За что отвечает |
|---|---|---|---|
| Глубина + цитаты | **DeepSeek V3.2** | $0.26 / $0.38 | анализ, нюансы, цитаты |
| Второе мнение + reasoning | **Kimi K2.5** (Moonshot) | $0.38 / $1.72 | свежий китайский трейн, другой взгляд |
| Цифры + факты | **Gemini 2.5 Pro** | $1.25 / $10 | 1M контекст, силён в источниках |
| Быстрый контрольный голос | **Grok 4 Fast** | $0.20 / $0.50 | подтверждение/опровержение фактов |
| Синтез + финальный отчёт | **Claude Sonnet 4.6** (ваша Claude Code подписка) | — | структура, эмпатия, язык |

**Можно дороже:** добавить GPT-5.4 ($2.50 / $15), Claude Opus ($15 / $75), Gemini 3.1 Pro ($2 / $12) — но для 90% задач избыточно.
...
- Exa MCP — уже подключён в проекте

**Желательно (для Метода Франкенштейна):**
- OpenAI MCP (чтобы дёргать ChatGPT Deep Research)
- Gemini MCP (чтобы дёргать Gemini)

**Если MCP не настроены** — скилл всё равно работает, но только на Claude + Exa + локальных источниках. Результат чуть беднее, но всё равно в 5-6 раз быстрее ручной схемы.
...
Если данных нет — идём на открытые источники (Exa + 3 модели).
...
Используй открытые источники через Exa. Не выдумывай цифры.
```
...
Если OpenAI MCP не настроен — субагент вместо этого идёт в Exa и ищет цитаты руками.
...
Если Gemini MCP не настроен — субагент идёт в Exa.
...
**"OpenAI/Gemini MCP не настроен"** → скилл работает в урезанном режиме, только Claude + Exa. Результат ~70% качества.
